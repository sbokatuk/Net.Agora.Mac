...
supports the current target framework and the previous one only. Verified for this repository:
    ...
    macOS       net8.0-macos, net9.0-macos       net10.0-macos
...
def target_frameworks(package: zipfile.ZipFile) -> set[str]:
    ...
    Usually read from the lib/<tfm>/ folders, but a dependency-only package - a product's
    cross-platform metapackage, e.g. Net.Agora.Video - has no lib/ at all and declares its target
    frameworks solely as nuspec dependency groups. Ignoring those would silently drop the .NET 10
    leg of the metapackage during the merge.
    """
    found = set()
    for name in package.namelist():
        parts = name.split("/")
